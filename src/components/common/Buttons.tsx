...
export const LogoutButton: React.FC<ButtonProps> = ({ className, ...props }) => {
  ...
  const text = language === 'zh' ? '退出登录' : 'Sign out';
...
export const DeleteButton: React.FC<ButtonProps & { text?: string }> = ({ className, text, ...props }) => {
  const { language } = useI18n();
  const buttonText = text ?? (language === 'zh' ? '删 除' : 'Delete');
...
export const ThemeButton: React.FC<ThemeButtonProps> = ({
  ...
  return (
    <button className={cn('btn-3d-uiverse', variantClass, className)} {...props}>
...
export const BackButton: React.FC<{ onClick: () => void; className?: string }> = ({ onClick, className }) => {
  const { language } = useI18n();
  const text = language === 'zh' ? '返回列表' : 'Back to list';
...
export const RefreshButton: React.FC<ButtonProps> = ({ children, className, ...props }) => {
  ...
  return (
    <button className={cn('btn-refresh-uiverse', className)} {...props}>
      ...
      {children || (language === 'zh' ? '刷新' : 'Refresh')}
    </button>
  );
};
