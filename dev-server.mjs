...
async function handlePassword(method, body) {
  ...
  if (!newPassword || newPassword.length < 6) {
    return json({ success: false, error: '新密码无效或过短（少于 6 位）' }, 400);
  }
  
  await ug_guard.put('admin_pwd', newPassword);
  return json({ success: true });
}
...
async function handleAppDetail(method, appId, body) {
  ...
  if (method === 'PUT') {
    ...
    if (body.status) {
      const ti = await ug_guard.get(`token_${appData.tokenHash}`, { type: 'json' });
      if (ti) {
        ti.status = body.status === 'active' ? 'active' : 'revoked';
        await ug_guard.put(`token_${appData.tokenHash}`, JSON.stringify(ti));
      }
    }
    const token = await ug_guard.get(`token_plain_${appId}`);
    return json({ success: true, data: { ...appData, token } });
  ...
  if (method === 'DELETE') {
    ...
    for (const fp of deviceList) await ug_guard.delete(`device_${appId}_${fp}`);
    await ug_guard.delete(`devices_${appId}`);
    await ug_guard.delete(`app_${appId}`);
    const list = (await ug_guard.get('apps_list', { type: 'json' })) || [];
    await ug_guard.put('apps_list', JSON.stringify(list.filter(id => id !== appId)));
    return json({ success: true });
  }
...
async function handleDevices(method, appId) {
  ...
  for (const fp of deviceList) {
    const d = await ug_guard.get(`device_${appId}_${fp}`, { type: 'json' });
    if (d) devices.push(d);
  }
...
async function handleDeviceUpdate(method, appId, deviceId, body) {
  ...
  for (const fp of deviceList) {
    ...
    if (d && d.deviceId === deviceId) {
      ...
      await ug_guard.put(`device_${appId}_${fp}`, JSON.stringify(d));
...
async function handleVerify(method, body, ip) {
  ...
  if (device) {
    ...
    if (appData.maxDevices > 0) {
      const dl = (await ug_guard.get(`devices_${appData.id}`, { type: 'json' })) || [];
    ...
    const dl = (await ug_guard.get(`devices_${appData.id}`, { type: 'json' })) || [];
    dl.push(fpHash);
    await ug_guard.put(`devices_${appData.id}`, JSON.stringify(dl));
  }
